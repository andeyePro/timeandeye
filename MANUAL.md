...
- **Pin button** (when not pinned, ⌘P), **Away** (keep tracking while you step
  away, ⌘⇧L), and **Stop** (⌘.). While Away is pinned, what's actually on
  screen is still recorded as evidence (never billed, never taught) - and
  when the Away stretch ends with a few minutes or more of recorded
  evidence, the popover offers to rebuild it on the spot (the Rebuild
  button opens Settings ▸ Maintenance ▸ **Away rescue** with that stretch
  already previewed; the same pane also rescues older stretches, for
  about 30 days, for you to confirm).
...
- **Switch to / Reassign.** "Switch to" starts a fresh session on the task you
  ...
  total. Clicking the mode label (or the current task title, or ⌘T) flips
  to the other mode for that open; right-click the label to change which
  mode is the default (also a Setting).
...
- **BECAUSE** - the source that fired (pinned, a learned email rule,
  ...
  entries are never moved — they're flagged in Posting health and Review,
  and marked ⚠ on the timeline with a tooltip saying where today's rules
  would file them; weaker contradictions appear as one "look mis-filed"
  row in Review with refile-all and dismiss. And when a correction YOU
  make could also move other entries, the timeline offers them on the
  spot: the view widens back to the first affected entry, everything
  irrelevant dims, the candidates show dashed outlines (all selected),
  click any - or drag across a stretch - to keep or drop them, and one
  approve moves the survivors - your word, one ⌘Z. If something LEARNED
...
it happened, which gesture taught it, and on which window. Click a
correction to see what it has since decided - how many entries, how much
time, and whether any of that was same-app pull rather than the exact
window. When the
