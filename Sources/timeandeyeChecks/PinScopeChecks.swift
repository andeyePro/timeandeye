...
func pinScopeChecks(_ c: Checks) {
    ...
    c.check("url identity is host + path segments, query dropped") {
        let id = PinScope.identity(of: urlSig("https://github.com/andeyePro/timeandeye/issues/42?tab=1"))
        try expectEq(id?.kind, .url)
        try expectEq(id?.segments ?? [], ["github.com", "andeyePro", "timeandeye", "issues", "42"])
    }

    c.check("url default selection is host + first path segment") {
        let segs = ["github.com", "andeyePro", "timeandeye", "issues", "42"]
    ...
    c.check("app identity is app name + title segments; default is app + window") {
        let sig = ActivitySignal(app: "Visual Studio Code",
                                 windowTitle: "Attributor.swift — timeandeye", timestamp: now)
        let id = PinScope.identity(of: sig)
        try expectEq(id?.kind, .app)
        try expectEq(id?.segments ?? [], ["Visual Studio Code", "Attributor.swift", "timeandeye"])
    ...
    c.check("a prefix pin matches any deeper identity, not a sibling") {
        ...
        try expect(pin.matches(urlSig("https://github.com/andeyePro/timeandeye/issues/42")))
    ...
    c.check("a more-specific app pin still requires all its segments present") {
        let pin = PinScope(kind: .app, prefix: ["Code", "Attributor.swift", "timeandeye"])
        func c2(_ t: String) -> ActivitySignal {
            ActivitySignal(app: "Code", windowTitle: t, timestamp: now)
        }
        try expect(pin.matches(c2("Attributor.swift — timeandeye")))
        try expect(pin.matches(c2("timeandeye — Attributor.swift — zsh")), "order-independent")
        try expect(!pin.matches(c2("Attributor.swift — other")), "missing 'timeandeye'")
    }
