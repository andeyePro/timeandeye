...
func predicateParserChecks(_ c: Checks) {
    ...
    func parsed(_ s: String) throws -> timeandeyeCore.Predicate {
    ...
    c.check("single leaf with each operator") {
        ...
        try expectEq(try parsed("title starts with \"timeandeye\""),
                     .leaf(field: .title, op: .startsWith, value: "timeandeye"))
    ...
    c.check("bare text means contains-any-field") {
        let p = try parsed("timeandeye")
        try expectEq(p, PredicateParser.anyFieldContains("timeandeye"))
    ...
    c.check("negation: 'is not' and 'does not contain'") {
        ...
        let p = try parsed("title contains \"timeandeye\" and app is not \"Ghostty\"")
        try expectEq(p, .and([
    ...
    c.check("render round-trips back to an equal predicate") {
        let cases: [timeandeyeCore.Predicate] = [
            .leaf(field: .app, op: .equals, value: "Ghostty"),
            .and([.leaf(field: .title, op: .contains, value: "timeandeye"),
