...
func journalStoreConformanceChecks(_ c: Checks, make: () -> any JournalStore) {
    ...
    c.check("spans round-trip with range query") {
        let s = make()
        let signal = ActivitySignal(app: "Ghostty", windowTitle: "timeandeye", timestamp: t0)
        ...
        try expectEq(hits.first?.signal.windowTitle, "timeandeye")
...
func opClientChecks(_ c: Checks) async {
    ...
    await c.check("fetchTasks pages through and parses") {
        ...
        transport.responses = [
            ...
            {"total": 3, "count": 2, "_embedded": {"elements": [
              {"id": 1, "subject": "timeandeye build",
               "_links": {"status": {"title": "Now"},
                          "project": {"title": "timeandeye", "href": "/api/v3/projects/14"}}},
        ...
        try expectEq(tasks[0], WorkTask(ref: .op(1), subject: "timeandeye build",
                                        project: "timeandeye", projectID: "14", status: "Now"))
    ...
    await c.check("createTimeEntry body") {
        ...
        try await makeClient(transport).createTimeEntry(
            ...
            activityID: 4, comment: "Ghostty – timeandeye")
        ...
        try expectEq((json["comment"] as? [String: String])?["raw"], "Ghostty – timeandeye")
...
func syncEngineChecks(_ c: Checks) async {
    ...
    await c.check("pushes eligible, marks the ledger AND the legacy mirror") {
        ...
        let eligible = Session(task: .op(42), start: t0, end: t0.addingTimeInterval(1800),
                               certainty: 0.9, comment: "Ghostty – timeandeye")
        ...
        try expectEq((body["comment"] as? [String: String])?["raw"], "Ghostty – timeandeye")
...
func aiAssistChecks(_ c: Checks) {
    ...
    let tasks = [WorkTask(ref: .op(1), subject: "timeandeye build",
                          project: "timeandeye", status: "Now")]
    ...
    c.check("prompt contains tasks, segments and format contract") {
        ...
        try expect(prompt.contains("timeandeye build"))
...
func settingsChecks(_ c: Checks) {
    c.check("JSONFileStore backs up each save and recovers the latest value from a corrupt main") {
        ...
        let store = JSONFileStore<AndeyeSettings>(url: url)
        try store.save(AndeyeSettings(opBaseURL: "https://op.example.com"))
        try store.save(AndeyeSettings(opBaseURL: "https://op2.example.com"))  // bak mirrors latest
        ...
        try store.save(AndeyeSettings(opBaseURL: "https://op3.example.com"))
    ...
    c.check("unknown/renamed enum values must NOT wipe the settings file (keeps the OP URL)") {
        ...
        """
        let s = try JSONDecoder().decode(AndeyeSettings.self, from: Data(json.utf8))
        let defs = AndeyeSettings(opBaseURL: "")
    ...
    c.check("defaults") {
        let s = AndeyeSettings(opBaseURL: "https://op.example.com")
    ...
    c.check("never-auto-push is representable") {
        var s = AndeyeSettings(opBaseURL: "https://op.example.com")
...
func endToEndChecks(_ c: Checks) async {
    await c.check("a full tracked stretch reaches OpenProject") {
        ...
        tracker.handle(.focus(ActivitySignal(app: "Ghostty", windowTitle: "timeandeye",
                                             timestamp: t(20))))
